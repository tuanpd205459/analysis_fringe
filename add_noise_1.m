...
% Tỉ lệ nhiễu (%)
noise_level = 1; % 5%
% --- Thêm nhiễu dựa trên std (khuyến nghị) ---
...
switch lower(phase_type)
    case 'peaks'
        ...
        noise = noise_level * std(object_phase_without_noise(:)) * randn(size(object_phase_without_noise));
%         noise = noise_level * mean(object_phase_without_noise(:)) * randn(size(object_phase_without_noise));
%         noise  =  noise_level * max(object_phase_without_noise(:))  * randn(size(object_phase_without_noise));
        object_phase = object_phase_without_noise + noise;
    case"sigma"
%         N = 200;
        [x, sigma] = meshgrid(linspace(0, 1, N), linspace(0, pi/10, N));
        ...
        eta = noise_level* randn(N, N) .* sigma;
        % Calculate the unwrapped phase distribution
        phi = a * x.^2 + eta;

        object_phase_without_noise = a*(x.^2);
        object_phase = phi;
    otherwise
...
% wrappedEstimate = wrapToPi(phi_est);
kMap = round((phi_est - wrapped_phase) / (2*pi));
finalUnwrappedPhase = wrapped_phase + 2*pi * kMap;
% fprintf("chay k map");
% kMap = kMap - min(kMap(:));
...
[object_phase_without_noise, object_phase, unwrapped_Phase_LS_DCT, unwrapped_Phase_TIE_FFT, unwrapped_Phase_noncontinue,...
    unwrapped_Phase_2dweight, unwrapped_Phase_goldstein, unwrapped_Phase_proposal]...
    = crop_multiple_to_smallest(object_phase_without_noise, object_phase, unwrapped_Phase_LS_DCT, unwrapped_Phase_TIE_FFT, unwrapped_Phase_noncontinue,...
    unwrapped_Phase_2dweight, unwrapped_Phase_goldstein, unwrapped_Phase_proposal);
...
phases = {object_phase_without_noise, unwrapped_Phase_LS_DCT, unwrapped_Phase_TIE_FFT, ...
          unwrapped_Phase_noncontinue, unwrapped_Phase_2dweight, ...
          unwrapped_Phase_goldstein, unwrapped_Phase_proposal};
...
legend('Ground Truth', 'LS+DCT', 'TIE+FFT', 'Non-continue', ...
...
% error_LS_DCT = unwrapped_Phase_LS_DCT - object_phase_without_noise;
% error_TIE_FFT = unwrapped_Phase_TIE_FFT - object_phase_without_noise;
% error_noncontinue = unwrapped_Phase_noncontinue - object_phase_without_noise;
% error_2dweight = unwrapped_Phase_2dweight - object_phase_without_noise;
% error_goldstein = unwrapped_Phase_goldstein - object_phase_without_noise;
% error_proposal = unwrapped_Phase_proposal - object_phase_without_noise;
...
for k = 1:nPhase
    ...
    offset = median(phase(:)) - median(object_phase_without_noise(:));
    phase_adj = phase - offset;
    
    % --- B2: Tính sai số
    diff = phase_adj - object_phase_without_noise;
...
for k = 1:nPhase
    ...
    % B1: Loại bỏ offset
    offset = median(object_phase_without_noise(:)) - median(object_phase_without_noise(:));
    phase_adj = phase - offset;
...
% --- Vẽ biểu đồ so sánh RMSE, MAE, MAX ---
figure;

% Lấy dữ liệu
rmse_vals = [errors.RMSE];
mae_vals  = [errors.MAE];
maxe_vals = [errors.MAX];

% Tên phương pháp
names = {errors.Name};

% Vẽ biểu đồ cột nhóm
bar_data = [rmse_vals(:), mae_vals(:), maxe_vals(:)];
bar(bar_data);
xticklabels(names);
xtickangle(30); % xoay nhãn cho dễ đọc
ylabel('Error value (rad)');
legend({'RMSE','MAE','MAX'}, 'Location','northoutside','Orientation','horizontal');
title('So sánh sai số giữa các phương pháp unwrapping');
grid on;
% --- Vẽ bản đồ sai số 3D ---
figure;
for k = 1:nPhase
...
%     phase_adj = phase - offset;
    ...
    % B2: Sai số tuyệt phase
    error_map = abs(phase - object_phase_without_noise);
...
sgtitle('Bản đồ sai số 3D tuyệt đối');
...
hologram_Gauss = awgn(hologram,snr,'measured','dB');% Thêm nhiễu Gaussian trắng
