...
phase_noise=phase_origin;
sub_colum=6;
sub_row=6;
foldername=['result_iter_speckle_',datestr(clock, 'dd-mmm-yyyy_HH-MM-SS')];
mkdir(foldername)
i=1;
 speckle_size=1;
 std=[0,0.5,1];
for k=1:3
 load (['data/wrapped_phase_with_size_' num2str(speckle_size) '_std_' num2str(std(k)) '.mat'])
tic
[phase_unwrap1(:,:,k)]=unwrap_LS_FD_FFT_iter(phase_wrap);
time1(k)=toc;
tic
[phase_unwrap2(:,:,k)]=unwrap_LS_FD_DCT_iter(phase_wrap);
time2(k)=toc;
tic
[phase_unwrap3(:,:,k)]=unwrap_TIE_FD_FFT_iter(phase_wrap);
time3(k)=toc;
tic
[phase_unwrap4(:,:,k)]=unwrap_TIE_FD_DCT_iter(phase_wrap);
time4(k)=toc;
tic
[phase_unwrap5(:,:,k)]=unwrap_TIE_FFT_FFT_iter(phase_wrap);
time5(k)=toc;
tic
[phase_unwrap6(:,:,k)]=unwrap_TIE_FFT_DCT_iter(phase_wrap);
time6(k)=toc;
