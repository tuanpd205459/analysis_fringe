...
% ảnh oke trong folde dataset3/26-11-25
% đã sửa 27/11/25
...
% Thông số đường dẫn
% Thư mục chứa ảnh
baseFolder = "D:\tuan\data 26 11 25\dataset3 -oke";
...
defaultFolder = 'D:\tuan\data 26 11 25\dataset3 -oke';
...
%% --- GIAI ĐOẠN CUỐI: NỐI BẰNG GIAO CẮT HÌNH HỌC ---
fprintf('--> Bước cuối: Nối các vân giao nhau (Intersecting Ridges)...\n');

% 1. Tính toán lại Endpoint và Vector lần cuối (vì BW đã thay đổi sau vòng lặp trên)
endPoints_mask = bwmorph(BW, 'endpoints');
vectors_data = fitEndpointVectors(BW, endPoints_mask, 12); 
% Giả sử fitEndpointVectors trả về [x, y, vx, vy]
...
%% cắt ảnh BW
% % do ảnh oke toàn miền, nên gần như không cần chọn vùng
% % thay vào đo là ofset cạnh
% [pos, xRec, yRec, widthRec, heightRec] = myDrawRec();
% 
% % --- 4. Cắt vùng ảnh đã chọn ---
% BW_connected = BW_connected(yRec : yRec + heightRec - 1, ...
...
% figure();
% imshow(BW_connected, []);
% title('BW connected Sau khi cat');
% hold on;
% endpoints = bwmorph(BW_connected, "endpoints");
...
title("Anh pha phi estimate");

%%
hologram = hologram(offset : end -offset +1, ...
                         offset : end -offset +1);

% do phanaf tren su dung offset, nen cho nay //comment
