% =========================================================================
% SCRIPT CHÍNH: MÔ PHỎNG, TÁI TẠO VÀ PHÂN TÍCH PHA - NÂNG CẤP TILT REMOVAL
% =========================================================================
%% 1. KHỞI TẠO
clc;
clear;
close all;
fprintf('Bắt đầu quy trình mô phỏng và tái tạo (Robust Tilt Removal)...\n');

%% 2. MÔ PHỎNG HOLOGRAM
fprintf('--> Bước 1: Mô phỏng Hologram...\n');
% --- Thiết lập thông số ---
M = 512; % Kích thước ảnh
N = 512;
...
% Lựa chọn: 'peaks', 'sin', 'gaussian', 'zernike'
phase_type = 'peaks';
switch lower(phase_type)
    ...
    case 'gaussian'
        sigma = 0.1;
        phi_ground_truth = 2 * exp(-(X.^2 + Y.^2) / (2 * sigma^2));
    case 'complex_surface'
        % Tạo bề mặt phức tạp với tilt lớn
        phi_ground_truth = 2 * peaks(3*X, 3*Y) + 5*X + 3*Y;
    otherwise
        error('Loại pha chưa được định nghĩa!');
end

% --- Tạo hologram từ pha gốc ---
hologram = generate_test_hologram(M, N, fx, fy, phi_ground_truth);
...
[wrappedPhase, ~] = reconstruct_phase_auto(hologram, struct());
...
% --- Căn chỉnh kích thước các bản đồ pha ---
offset_val = 10;
...
[initialUnwrappedPhase, kMap] = unwrapUsingEstimate(phi_est_aligned, wrappedPhase_aligned);

%% 6. ROBUST TILT REMOVAL (PHƯƠNG PHÁP MỚI)
fprintf('--> Bước 5: Xóa nghiêng bằng RANSAC + Zernike...\n');
% --- Chuẩn bị input ---
% Tạo mask tự động (circular aperture)
[X_mask, Y_mask] = meshgrid(linspace(-1, 1, size(initialUnwrappedPhase, 2)), ...
                           linspace(-1, 1, size(initialUnwrappedPhase, 1)));
circular_mask = (X_mask.^2 + Y_mask.^2) <= 0.9; % Hơi nhỏ hơn để tránh biên

% --- Áp dụng robust tilt removal ---
tic;
[finalUnwrappedPhase, tilt_info] = robust_tilt_removal(initialUnwrappedPhase, ...
    'Mask', circular_mask, ...
    'MaxIterations', 2000, ...
    'Threshold', 0.3, ...
    'ZernikeOrder', 15, ...
    'Confidence', 0.99, ...
    'Verbose', true);
tilt_removal_time = toc;

fprintf('Thời gian xóa nghiêng: %.2f giây\n', tilt_removal_time);
fprintf('Tilt X đã xóa: %.4f rad/pixel\n', tilt_info.removed_tilt_x);
fprintf('Tilt Y đã xóa: %.4f rad/pixel\n', tilt_info.removed_tilt_y);
fprintf('Tỷ lệ inlier: %.1f%%\n', tilt_info.inlier_ratio * 100);

%% 7. POST-PROCESSING BỔ SUNG
fprintf('--> Bước 6: Xử lý các artifact còn lại...\n');
% Xử lý sparse artifacts
...
% Cắt bỏ border nếu cần
finalUnwrappedPhase = finalUnwrappedPhase(2:end-1, 2:end-1);
phi_ground_truth_aligned = phi_ground_truth_aligned(2:end-1, 2:end-1);
circular_mask_cropped = circular_mask(2:end-1, 2:end-1);

%% 8. PHÂN TÍCH SAI SỐ CHI TIẾT
fprintf('--> Bước 7: Phân tích sai số chi tiết...\n');

% Tính sai số RMS chỉ trong vùng có valid
valid_region = circular_mask_cropped & ~isnan(finalUnwrappedPhase) & ~isnan(phi_ground_truth_aligned);

if any(valid_region(:))
    error_map = finalUnwrappedPhase - phi_ground_truth_aligned;
    error_in_valid = error_map(valid_region);
    
    rmse_final = sqrt(mean(error_in_valid.^2));
    mae_final = mean(abs(error_in_valid));
    max_error = max(abs(error_in_valid));
    
    % Phân tích thống kê lỗi
    error_std = std(error_in_valid);
    error_median = median(error_in_valid);
    
    fprintf('=============================\n');
else
    fprintf('Cảnh báo: Không có pixel valid để tính sai số!\n');
    rmse_final = NaN;
end

%% 9. HIỂN THỊ KẾT QUẢ NÂNG CẤP
fprintf('--> Bước 8: Hiển thị kết quả...\n');

% Tạo figure tổng quan
figure('Name', 'Tổng Quan - Robust Tilt Removal', 'Position', [100, 100, 1200, 800]);

% Pha gốc
subplot(2, 3, 1);
imagesc(phi_ground_truth_aligned, 'AlphaData', circular_mask_cropped);
...
% Pha wrapped
subplot(2, 3, 2);
imagesc(wrappedPhase_aligned, 'AlphaData', circular_mask_cropped);
title('Pha Wrapped');
...
% Kết quả cuối cùng
subplot(2, 3, 3);
imagesc(finalUnwrappedPhase, 'AlphaData', circular_mask_cropped);
...
% Bản đồ sai số
subplot(2, 3, 4);
if exist('error_map', 'var')
    imagesc(error_map, 'AlphaData', circular_mask_cropped);
    title(sprintf('Bản Đồ Sai Số (RMS=%.4f)', rmse_final));
...
% Histogram sai số
subplot(2, 3, 5);
if exist('error_in_valid', 'var')
    histogram(error_in_valid, 50, 'Normalization', 'pdf');
...
% Thông tin tilt
subplot(2, 3, 6);
text(0.1, 0.9, sprintf('Tilt X: %.4f rad/pixel', tilt_info.removed_tilt_x), ...
     'Units', 'normalized', 'FontSize', 10);
text(0.1, 0.8, sprintf('Tilt Y: %.4f rad/pixel', tilt_info.removed_tilt_y), ...
     'Units', 'normalized', 'FontSize', 10);
text(0.1, 0.7, sprintf('Inlier ratio: %.1f%%', tilt_info.inlier_ratio*100), ...
     'Units', 'normalized', 'FontSize', 10);
text(0.1, 0.6, sprintf('RMS residual: %.4f', tilt_info.rms_residual), ...
     'Units', 'normalized', 'FontSize', 10);
text(0.1, 0.5, sprintf('Final RMS: %.4f rad', rmse_final), ...
     'Units', 'normalized', 'FontSize', 10);
text(0.1, 0.4, sprintf('Processing time: %.2f s', tilt_removal_time), ...
     'Units', 'normalized', 'FontSize', 10);
title('Thông Tin Xử Lý');
axis off;

% Hiển thị bản đồ sai số chi tiết
figure('Name', 'Phân Tích Sai Số Chi Tiết', 'Position', [50, 50, 800, 600]);
if exist('error_map', 'var')
    surf(error_map, 'EdgeColor', 'none');
    title({'Bản Đồ Sai Số 3D', sprintf('RMS = %.4f rad', rmse_final)});
    xlabel('X'); ylabel('Y'); zlabel('Sai số (rad)');
    colormap('hot'); colorbar;
end

%% 10. EXPORT KẾT QUẢ
fprintf('--> Bước 9: Xuất kết quả...\n');
% Lưu các kết quả chính
results = struct();
results.finalPhase = finalUnwrappedPhase;
results.groundTruth = phi_ground_truth_aligned;
results.errorMap = error_map;
results.tiltInfo = tilt_info;
results.metrics = struct('rmse', rmse_final, 'mae', mae_final, 'max_error', max_error);
results.processingTime = tilt_removal_time;

% Xuất file
save('robust_tilt_removal_results.mat', 'results');

% Xuất ảnh kết quả
imwrite(mat2gray(finalUnwrappedPhase), 'final_phase.png');
if exist('error_map', 'var')
    imwrite(mat2gray(abs(error_map)), 'error_map.png');
end

fprintf('\nQuy trình hoàn tất với phương pháp Robust Tilt Removal!\n');
fprintf('Các file đã được lưu: robust_tilt_removal_results.mat, final_phase.png\n');
%%
function [corrected_phase, tilt_params] = robust_tilt_removal(phase_data, varargin)
% ROBUST_TILT_REMOVAL - Xóa góc nghiêng sử dụng RANSAC + Zernike
%
% Inputs:
%   phase_data - Ma trận pha cần xử lý
%   varargin - Các tham số tùy chọn:
%     'MaxIterations' - Số lần lặp tối đa RANSAC (default: 2000)
%     'Threshold' - Ngưỡng inlier (default: 0.5)
%     'ZernikeOrder' - Bậc Zernike (default: 15)
%     'Confidence' - Độ tin cậy (default: 0.99)
%     'Mask' - Mặt nạ aperture (default: auto-generated)
%
% Outputs:
%   corrected_phase - Pha đã xóa nghiêng
%   tilt_params - Thông tin về tilt đã xóa

    % Parse input arguments
    p = inputParser;
    addRequired(p, 'phase_data', @isnumeric);
    addParameter(p, 'MaxIterations', 2000, @isnumeric);
    addParameter(p, 'Threshold', 0.5, @isnumeric);
    addParameter(p, 'ZernikeOrder', 15, @isnumeric);
    addParameter(p, 'Confidence', 0.99, @isnumeric);
    addParameter(p, 'Mask', [], @(x) islogical(x) || isempty(x));
    addParameter(p, 'Verbose', false, @islogical);
    
    parse(p, phase_data, varargin{:});
    
    max_iterations = p.Results.MaxIterations;
    threshold = p.Results.Threshold;
    zernike_order = p.Results.ZernikeOrder;
    ...
    % Tạo mặt nạ aperture nếu không có
    if isempty(mask)
        [X, Y] = meshgrid(linspace(-1, 1, N), linspace(-1, 1, M));
        mask = (X.^2 + Y.^2) <= 1;
    end
    
    % Tạo lưới tọa độ cho các điểm valid
    [x_grid, y_grid] = meshgrid(1:N, 1:M);
    valid_idx = mask & ~isnan(phase_data);
    
    x_valid = x_grid(valid_idx);
    y_valid = y_grid(valid_idx);
    z_valid = phase_data(valid_idx);
    
    if length(z_valid) < 3
        error('Không đủ điểm valid để fit mặt phẳng');
    end
    
    %% BƯỚC 1: RANSAC PLANE FITTING
    if verbose
        fprintf('Bắt đầu RANSAC plane fitting...\n');
    end
    ...
    % Tính số lần lặp cần thiết dựa trên confidence
    outlier_ratio = 0.3; % Ước lượng tỷ lệ outlier
    needed_iterations = log(1 - confidence) / log(1 - (1 - outlier_ratio)^3);
    iterations = min(max_iterations, ceil(needed_iterations));
    
    for iter = 1:iterations
        % Chọn 3 điểm ngẫu nhiên
        sample_idx = randperm(length(z_valid), 3);
        sample_x = x_valid(sample_idx);
        sample_y = y_valid(sample_idx);
        sample_z = z_valid(sample_idx);
        
        % Fit mặt phẳng z = ax + by + c
        A = [sample_x, sample_y, ones(3,1)];
        if rank(A) < 3
            continue; % Skip nếu 3 điểm thẳng hàng
        end
        
        plane_params = A \ sample_z;
        a = plane_params(1);
        b = plane_params(2);
        c = plane_params(3);
        
        % Tính residual cho tất cả điểm
        predicted_z = a * x_valid + b * y_valid + c;
        residuals = abs(z_valid - predicted_z);
        ...
        % Đánh giá chất lượng
        score = num_inliers / length(z_valid);
    ...
    if isempty(best_plane)
        error('RANSAC không tìm được mặt phẳng phù hợp');
    end
    
    % Refine plane với inliers
    predicted_z = best_plane(1) * x_valid + best_plane(2) * y_valid + best_plane(3);
    residuals = abs(z_valid - predicted_z);
    inliers = residuals < threshold;
    
    % Least squares refinement với inliers
    A_inliers = [x_valid(inliers), y_valid(inliers), ones(sum(inliers), 1)];
    refined_plane = A_inliers \ z_valid(inliers);
    
    if verbose
        fprintf('RANSAC hoàn thành: %d/%d inliers (%.1f%%)\n', ...
                best_inliers, length(z_valid), best_score*100);
    end
    
    %% BƯỚC 2: XÓA TILT CƠ BẢN
    coarse_corrected = phase_data;
    for i = 1:M
        for j = 1:N
            if mask(i, j)
                predicted_tilt = refined_plane(1) * j + refined_plane(2) * i + refined_plane(3);
                coarse_corrected(i, j) = phase_data(i, j) - predicted_tilt;
            end
        end
    end
    
    %% BƯỚC 3: ZERNIKE FINE TUNING
    if verbose
        fprintf('Bắt đầu Zernike refinement...\n');
    end
    ...
    % Tính các polynomial Zernike
    zernike_modes = generate_zernike_modes(X, Y, mask, zernike_order);
    
    % Phân tích Zernike
    phase_vector = coarse_corrected(mask);
    zernike_coeffs = zernike_modes \ phase_vector;
    
    % Xóa tilt components (Z2, Z3)
    if length(zernike_coeffs) >= 3
        zernike_coeffs(2) = 0; % Z2: tilt về x
        zernike_coeffs(3) = 0; % Z3: tilt về y
    end
    
    % Tái tạo pha đã hiệu chỉnh
    corrected_phase_vector = zernike_modes * zernike_coeffs;
    
    % Chuyển về dạng ma trận
    corrected_phase = coarse_corrected;
    corrected_phase(mask) = corrected_phase_vector;
    
    %% BƯỚC 4: POST-PROCESSING
    % Làm mượt nhẹ ở biên
    se = strel('disk', 1);
    boundary_mask = mask & ~imerode(mask, se);
    
    if any(boundary_mask(:))
        corrected_phase = smooth_boundary_region(corrected_phase, mask, boundary_mask);
    end
    
    %% THÔNG TIN OUTPUT
    tilt_params = struct();
    tilt_params.ransac_plane = refined_plane;
    tilt_params.inlier_ratio = best_score;
    tilt_params.zernike_coeffs = zernike_coeffs;
    tilt_params.removed_tilt_x = refined_plane(1);
    tilt_params.removed_tilt_y = refined_plane(2);
    tilt_params.rms_residual = sqrt(mean((corrected_phase_vector - phase_vector).^2));
    
    if verbose
        fprintf('Tilt removal hoàn thành!\n');
        fprintf('Tilt X: %.4f, Tilt Y: %.4f\n', refined_plane(1), refined_plane(2));
        fprintf('RMS residual: %.4f\n', tilt_params.rms_residual);
    end
end

%% HELPER FUNCTIONS
function zernike_modes = generate_zernike_modes(X, Y, mask, max_order)
    % Tạo các mode Zernike
    rho = sqrt(X.^2 + Y.^2);
    theta = atan2(Y, X);
    
    % Chỉ tính cho các điểm trong mask
    valid_points = sum(mask(:));
    zernike_modes = zeros(valid_points, max_order);
    
    rho_valid = rho(mask);
    theta_valid = theta(mask);
    
    % Tạo các polynomial Zernike cơ bản
    mode_idx = 1;
    
    % Z1: Piston
    zernike_modes(:, mode_idx) = ones(valid_points, 1);
    mode_idx = mode_idx + 1;
    
    % Z2: Tilt X
    if mode_idx <= max_order
        zernike_modes(:, mode_idx) = rho_valid .* cos(theta_valid);
        mode_idx = mode_idx + 1;
    end
    
    % Z3: Tilt Y
    if mode_idx <= max_order
        zernike_modes(:, mode_idx) = rho_valid .* sin(theta_valid);
        mode_idx = mode_idx + 1;
    end
    
    % Z4: Defocus
    if mode_idx <= max_order
        zernike_modes(:, mode_idx) = 2*rho_valid.^2 - 1;
        mode_idx = mode_idx + 1;
    end
    
    % Z5: Astigmatism (45°)
    if mode_idx <= max_order
        zernike_modes(:, mode_idx) = rho_valid.^2 .* sin(2*theta_valid);
        mode_idx = mode_idx + 1;
    end
    
    % Z6: Astigmatism (0°)
    if mode_idx <= max_order
        zernike_modes(:, mode_idx) = rho_valid.^2 .* cos(2*theta_valid);
        mode_idx = mode_idx + 1;
    end
    
    % Thêm các mode bậc cao hơn nếu cần
    for n = 3:floor(sqrt(max_order))
        for m = -n:2:n
            if mode_idx > max_order
                break;
            end
            ...
            if m >= 0
                zernike_modes(:, mode_idx) = zernike_radial(rho_valid, n, m) .* cos(m * theta_valid);
            else
                zernike_modes(:, mode_idx) = zernike_radial(rho_valid, n, abs(m)) .* sin(abs(m) * theta_valid);
            end
    ...
    % Cắt bớt nếu cần
    zernike_modes = zernike_modes(:, 1:min(max_order, mode_idx-1));
end

function R = zernike_radial(rho, n, m)
    % Tính radial polynomial Zernike
    R = zeros(size(rho));
...
function phase_smooth = smooth_boundary_region(phase, mask, boundary_mask)
    % Làm mượt vùng biên
    phase_smooth = phase;
    
    % Gaussian smoothing chỉ ở biên
    smooth_kernel = fspecial('gaussian', [5 5], 1.5);
    phase_boundary_smooth = imfilter(phase, smooth_kernel, 'symmetric');
    
    % Áp dụng chỉ ở vùng biên
    phase_smooth(boundary_mask) = phase_boundary_smooth(boundary_mask);
end
...
%% thêm 9-7-25
function [phase_detrended, plane] = remove_tilt_least_squares(phase_input, mask)
% Loại bỏ mặt phẳng nghiêng tuyến tính khỏi pha bằng bình phương tối thiểu
% - phase_input: bản đồ pha liên tục
% - mask: (tùy chọn) vùng chọn để fit mặt phẳng (nếu không truyền thì dùng toàn ảnh)

    if nargin < 2
        mask = true(size(phase_input));
    end

    [Ny, Nx] = size(phase_input);
    [X, Y] = meshgrid(1:Nx, 1:Ny);

    x = X(mask); y = Y(mask); z = phase_input(mask);

    % Fit mặt phẳng: z = a*x + b*y + c
    A = [x(:), y(:), ones(size(x(:)))];
    coeff = A \ z(:);  % Least squares

    % Tạo mặt phẳng để trừ ra
    plane = coeff(1)*X + coeff(2)*Y + coeff(3);
    phase_detrended = phase_input - plane;
end
