...
% --- Thêm tham số chọn loại bề mặt mô phỏng ---
% Các lựa chọn: 'complex_wave', 'conical', 'step_discontinuity'
surface_type = 'conical'; % THAY ĐỔI LOẠI BỀ MẶT MÔ PHỎNG TẠI ĐÂY

% Tạo pha thực (true phase) - dạng bề mặt phức tạp
true_phase = create_complex_phase_surface(X, Y, N, surface_type);
...
%% FUNCTIONS
function true_phase = create_complex_phase_surface(X, Y, N, surface_type)
    % Tạo bề mặt pha phức tạp mô phỏng đo biến dạng thực tế
    % surface_type: 'complex_wave', 'conical', 'step_discontinuity'
    
    switch lower(surface_type)
        case 'complex_wave'
            % Bề mặt phức tạp ban đầu (sóng, biến dạng cục bộ, gradient)
            ...
            fprintf('Tạo pha thực: Bề mặt sóng phức tạp.\n');

        case 'conical'
            % Bề mặt hình nón
            R = sqrt((X - N/2).^2 + (Y - N/2).^2);
            true_phase = 5 * R / (N/2) + 2 * sin(2*pi*X/30); % Thêm gợn sóng để phức tạp hơn
            true_phase = 5 * R / (N/2);
            fprintf('Tạo pha thực: Bề mặt hình nón.\n');

        case 'step_discontinuity'
            % Bề mặt có bước nhảy đột ngột
            true_phase = 2 * sin(2*pi*X/40) + 3 * cos(2*pi*Y/50); % Bề mặt cơ bản
            step_location_x = N/2;
            step_height = 8 * pi; % Bước nhảy lớn hơn 2pi
            
            % Áp dụng bước nhảy
            true_phase(X > step_location_x) = true_phase(X > step_location_x) + step_height;
            fprintf('Tạo pha thực: Bề mặt có bước nhảy.\n');

        otherwise
            error('Loại bề mặt không hợp lệ. Vui lòng chọn "complex_wave", "conical", hoặc "step_discontinuity".');
    end
    
    fprintf('Pha thực có range: %.2f đến %.2f rad\n', ...
...
function phase_estimate = simulate_fringe_phase_estimate(true_phase, X, Y)
    ...
    phase_estimate = true_phase + bias;
...
function display_simulation_data(true_phase, phase_wrapped, phase_estimate)
    ...
    colorbar;
    title('Pha Wrapped (từ FFT - đã lọc)'); 
    axis equal tight;
...
function display_final_results(true_phase, phase_wrapped, phase_estimate, ...
    ...
    imagesc(phase_final);
    colorbar;
    title('Pha sau unwrapping và xử lý nhiễu'); % Cập nhật tiêu đề
    axis equal tight;
    ...
    legend('True', 'Wrapped (đã lọc)', 'Estimate', 'Final (đã xử lý nhiễu)', 'Location', 'best'); % Cập nhật nhãn
    ...
    legend('Sai số Wrapped (đã lọc)', 'Sai số Estimate', 'Sai số Final (đã xử lý nhiễu)', 'Location', 'best'); % Cập nhật nhãn
    title('Sai số tuyệt đối');
    xlabel('Pixel');
...
function display_3d_views(true_phase, phase_wrapped, phase_final)
    ...
    shading interp;
    colorbar;
    title('Pha sau unwrapping và xử lý nhiễu (3D)'); % Cập nhật tiêu đề
    xlabel('X'); ylabel('Y'); zlabel('Phase (rad)');
    axis tight;
    view(-35, 45);
