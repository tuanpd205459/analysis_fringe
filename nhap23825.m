clc, clear, close all;
%% 1. Khởi tạo các tham số mô phỏng
N = 512;
M= 512;
...
% Calculate the ground truth unwrapped phase distribution
object_phase = 1 * x.^2 + eta;
%% 3. Tạo pha đã cuộn (wrapped phase)
phi_wrapped = mod(object_phase + pi, 2*pi) - pi;
%%
% 3. TẠO HOLOGRAM
...
wrapped_phase = phi_wrapped;
kMap = round((phi_est - wrapped_phase) / (2*pi));
finalUnwrappedPhase = wrapped_phase + 2*pi * kMap;
% [phi_unwrap_est, N_est] = Unwrap_TIE_DCT_Iter_Est(phi_wrapped, phi_est);


%% 4. Thực hiện giải cuộn pha (đoạn mã của bạn)
unwrapped_Phase_LS_DCT = unwrapping.unwrapPhase(phi_wrapped, 'ls', 'dct'); % LS với DCT
unwrapped_Phase_TIE_FFT = unwrapping.unwrapPhase(phi_wrapped, 'tie', 'fft'); % TIE với FFT
unwrapped_Phase_noncontinue = unwrapping.unwrapPhase(phi_wrapped, 'linh'); % Phương pháp của a Linh
unwrapped_Phase_2dweight = unwrapping.unwrapPhase(phi_wrapped, '2dweight'); % 2D weighted phase unwrapping
unwrapped_Phase_proposal = finalUnwrappedPhase;

%% 5. Tính toán và hiển thị sai số RMS
% Tính sai số cho từng phương pháp
error_LS_DCT = unwrapped_Phase_LS_DCT - phi_wrapped;
error_TIE_FFT = unwrapped_Phase_TIE_FFT - phi_wrapped;
error_noncontinue = unwrapped_Phase_noncontinue - phi_wrapped;
error_2dweight = unwrapped_Phase_2dweight - phi_wrapped;
% Tạo các hình ảnh 2D hiển thị sai số
figure;
subplot(2,2,1);
...
xlabel('x');
ylabel('\sigma');
% Thêm tiêu đề chung cho tất cả các ảnh
sgtitle('RMS Error Maps');
%% --- Hiển thị bề mặt 3D của các kết quả ---
figure('Name','So sánh bề mặt 3D sau unwrapping','Position',[100 100 1200 800]);
% subplot(2,3,1);
% surf(phi_unwrapped);
% shading interp; colorbar;
% xlabel('x'); ylabel('\sigma'); zlabel('\phi (rad)');
% title('Ground truth (Unwrapped)');
...
view(45,30); set(gca,'FontSize',11);
subplot(2,3,3);
surf(unwrapped_Phase_LS_DCT);
shading interp; colorbar;
xlabel('x'); ylabel('\sigma'); zlabel('\phi (rad)');
title('LS-DCT');
view(45,30); set(gca,'FontSize',11);
subplot(2,3,4);
surf(unwrapped_Phase_TIE_FFT);
shading interp; colorbar;
xlabel('x'); ylabel('\sigma'); zlabel('\phi (rad)');
title('TIE-FFT');
view(45,30); set(gca,'FontSize',11);
subplot(2,3,5);
surf(unwrapped_Phase_noncontinue);
shading interp; colorbar;
xlabel('x'); ylabel('\sigma'); zlabel('\phi (rad)');
title('Non-continue (Linh)');
view(45,30); set(gca,'FontSize',11);
subplot(2,3,6);
surf(unwrapped_Phase_2dweight);
shading interp; colorbar;
xlabel('x'); ylabel('\sigma'); zlabel('\phi (rad)');
title('2D weighted');
view(45,30); set(gca,'FontSize',11);
