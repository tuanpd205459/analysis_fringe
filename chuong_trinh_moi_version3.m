...
[finalUnwrappedPhase, num_iter, history] = correct_sparse_artifacts_iterative(finalUnwrappedPhase, ...
    'FilterSize', [20 20], 'Epsilon', 1e-5, 'MaxIterations', 30, 'Verbose', true);
...
% Hàm cải tiến: Xử lý các điểm nhiễu sparse với thuật toán lặp
% Dựa trên phương pháp lọc trung vị để xác định và hiệu chỉnh các điểm lỗi.
...
%   convergence_history - Lịch sử hội tụ (RMS của delta_k)
    ...
    if verbose
        fprintf('Bắt đầu quá trình hiệu chỉnh lặp...\n');
    ...
    for iter = 1:max_iterations
        ...
        filtered_phase = medfilt2(current_phase, filter_size);
        
        % Bước 2: Tính toán sự khác biệt về "thứ tự vân" 
        % delta_k = Round[(Phi_filtered - Phi_current) / 2π]
        delta_k = round((filtered_phase - current_phase) / (2*pi));
        
        % Tính toán metric hội tụ (RMS của delta_k)
        rms_delta_k = sqrt(mean(delta_k(:).^2));
        convergence_history(end+1) = rms_delta_k;
        num_iterations = iter;
        
        if verbose
            fprintf('Iteration %d: RMS(delta_k) = %.6f, Unique corrections: %d\n', ...
                    iter, rms_delta_k, length(unique(delta_k(:))));
        end
