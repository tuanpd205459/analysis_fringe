...
surf(be_mat_tai_tao,"EdgeColor","none");
...
% figure;
% imagesc(be_mat_tai_tao);

z_map = wavefront + 0.2 * randn(size(X));
% z_map = wavefront;
% x = linspace(-5, 5, 100);          % Trục X từ -5 đến 5
% y = linspace(-5, 5, 100);          % Trục Y từ -5 đến 5
% [X, Y] = meshgrid(x, y);           % Tạo lưới tọa độ

% % Tạo bề mặt bất kỳ bằng cách kết hợp sóng sin và gaussian noise
% Z = sin(sqrt(X.^2 + Y.^2)) + 0 * randn(size(X)); % Bề mặt dao động với nhiễu
% z_map = Z;
%% m, n indices
coeff = zeros(1, 2);
